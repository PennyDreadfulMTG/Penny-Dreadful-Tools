...
class Completionist(BooleanAchievement):
    ...
    sql = "CASE WHEN COUNT(CASE WHEN ct.name = 'League' THEN 1 ELSE NULL END) > 0 AND COUNT(CASE WHEN d.retired = 1 AND (COALESCE(dc.wins, 0) + COALESCE(dc.draws, 0) + COALESCE(dc.losses, 0)) < 5 THEN 1 ELSE NULL END) = 0 THEN True ELSE False END"
