...
def load_cards(season_id: Optional[int] = None, person_id: Optional[int] = None, retry: bool = False) -> List[Card]:
    ...
    sql = """
        SELECT
            ...
            SUM(num_decks_sideboard) AS num_decks_sideboard,
            SUM(wins) AS wins,
            SUM(losses) AS losses,
            SUM(draws) AS draws,
            SUM(wins - losses) AS record,
            SUM(num_decks_tournament) AS num_decks_tournament,
            SUM(num_decks_sideboard_tournament) AS num_decks_sideboard_tournament,
...
def preaggregate_card() -> None:
    ...
    db().execute("""
        CREATE TABLE IF NOT EXISTS _new_card_stats (
            ...
            num_decks_sideboard INT NOT NULL,
            wins INT NOT NULL,
            losses INT NOT NULL,
            draws INT NOT NULL,
            ...
            num_decks_sideboard_tournament INT NOT NULL,
        ...
        SELECT
            ...
            SUM(CASE WHEN (d.id IS NOT NULL) AND (dc.n_side != 0) THEN 1 ELSE 0 END) AS num_decks_sideboard,
            ...
            SUM(CASE WHEN (d.id IS NOT NULL) AND (ct.name = 'Gatherling') AND 
                          (dc.n_side != 0) THEN 1 ELSE 0 END) AS num_decks_sideboard_tournament,
