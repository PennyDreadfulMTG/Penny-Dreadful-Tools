...
def tournament_deck(cells, competition_id, date, ranks):
    ...
    if len(d['cards']) == 0:
        logger.warn('Rejecting deck with id {id} because it has no cards.'.format(id=gatherling_id))
        return None
...
def tournament_matches(d):
    ...
    if anchor is None:
        logger.warn('Skipping {id} because it has no MATCHUPS.'.format(id=d.id))
        return []
...
def find_matches(d, rows):
    ...
    for row in rows:
        ...
        if 'No matches were found for this deck' in tds[0].renderContents().decode('utf-8'):
            logger.warn('Skipping {identifier} because it played no matches.'.format(identifier=d.identifier))
            break
