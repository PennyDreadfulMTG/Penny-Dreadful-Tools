...
class SmokeTest(unittest.TestCase):
    ...
    def test_home_data(self):
        result = self.app.get('/')
        self.assertIn('<h1>Latest Decks</h1>', result.data.decode('utf-8'))
    ...
    def test_some_pages(self):
        ...
        self.assertEqual(result.status_code, 200)
        result = self.app.get('/archetypes/1/')
        self.assertEqual(result.status_code, 200)
        result = self.app.get('/people/')
        self.assertEqual(result.status_code, 200)
        result = self.app.get('/people/1/')
        self.assertEqual(result.status_code, 200)
        ...
        result = self.app.get('/competitions/')
        self.assertEqual(result.status_code, 200)
        result = self.app.get('/competitions/1/')
        self.assertEqual(result.status_code, 200)
