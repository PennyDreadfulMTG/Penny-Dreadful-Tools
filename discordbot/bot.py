...
class Bot(commands.Bot):
    ...
    async def on_ready(self) -> None:
        ...
        logging.info(f'Connected to {names}')
        logging.info('--------')
    ...
    async def background_task_tournaments(self) -> None:
        ...
        if not isinstance(channel, discord.abc.Messageable):
            logging.warning(f'ERROR: could not find tournament_channel_id {tournament_channel_id}')
            return
        while self.is_ready:
    ...
    async def background_task_league_end(self) -> None:
        ...
        while self.is_ready:
            try:
                ...
                logging.error(f"Couldn't reach decksite or decode league json with error message(s) {err}")
