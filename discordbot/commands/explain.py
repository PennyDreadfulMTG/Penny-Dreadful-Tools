...
class ExplainCog(commands.Cog):
    def __init__(self, bot: commands.Bot) -> None:
        ...
        self.explanations: Dict[str, Tuple[str, Dict[str, str]]] = {
            'archetype': (
                """
        ...
        self.explanations['legality'] = self.explanations['rotation']
        ...
        self.slash_explain = cog_ext.cog_slash(
            name='explain',
            ...
            options=[
                create_option(
                    ...
                    option_type=3,
                    required=False,
                    choices=[x.lower() for x in self.keys]),
            ],
        ...
        try:
            if word == 'reporting':
                if is_tournament_channel(ctx.channel):
                    explanation = self.reporting_explanations['tournament']
                else:
                    explanation = self.reporting_explanations['league']
            elif word == 'promos':
                explanation = promo_explanation()
            else:
                explanation = self.explanations[word]

            s = '{text}\n'.format(text=textwrap.dedent(explanation[0]))
        except KeyError:
            usage = 'I can explain any of these things: {things}'.format(
                things=', '.join(sorted(self.keys)))
...
def is_tournament_channel(channel: Union[TextChannel, DMChannel, GroupChannel]) -> bool:
