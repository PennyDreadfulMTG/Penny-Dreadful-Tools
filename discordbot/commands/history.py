...
def card_history(c: Card) -> str:
    ...
    for format_name, status in c.legalities.items():
        if 'Penny Dreadful ' in format_name and status == 'Legal':
            ...
            history[season_id] = True
    history[seasons.current_season_num()] = c.legalities.get('Penny Dreadful', None) == 'Legal'
    s = '   '
    for i in range(1, seasons.current_season_num() + 1):
        s += f'{i} '
        s += ':white_check_mark:' if history.get(i, False) else ':no_entry_sign:'
        s += '   '
    s = s.strip()
