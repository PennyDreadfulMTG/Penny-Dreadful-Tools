import asyncio
import random

from interactions import Client, Extension
from interactions.models import slash_command

from discordbot.command import MtgInteractionContext, complex_search
from magic import oracle
from magic.models import Card


class Rhinos(Extension):
    @slash_command()
    async def rhinos(self, ctx: MtgInteractionContext) -> None:
        """Anything can be a rhino if you try hard enough"""
        await ctx.defer()
        rhino_name = 'Siege Rhino'
        if random.random() < 0.05:
            rhino_name = 'Abundant Maw'
        og_rhino = oracle.cards_by_name()[rhino_name]

        def find_rhino(query: str) -> Card:
            cards = complex_search(f'f:pd {query}')
            if len(cards) == 0:
                cards = complex_search(query)
            return random.choice(cards)
        copy_rhino = await asyncio.to_thread(find_rhino, 'o:"copy of target creature"')
        zombie_rhino = await asyncio.to_thread(find_rhino, 'o:"return target creature card from your graveyard to the battlefield"')
        tutor_rhino = await asyncio.to_thread(find_rhino, 'o:"search your library for a creature"')
        msg = f'\nSo of course we have {og_rhino.name}.'
        msg += f" And we have {copy_rhino.name}. It can become a rhino, so that's a rhino."
        msg += f" Then there's {zombie_rhino.name}. It can get back one of our rhinos, so that's a rhino."
        msg += f" And then we have {tutor_rhino.name}. It's a bit of a stretch, but that's a rhino too."
        await ctx.post_cards([og_rhino, copy_rhino, zombie_rhino, tutor_rhino], additional_text=msg)

def setup(bot: Client) -> None:
    Rhinos(bot)
