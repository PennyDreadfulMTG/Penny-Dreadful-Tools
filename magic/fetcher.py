...
async def all_cards_async(force_last_good: bool = False) -> tuple[list[CardDescription], str]:
    ...
    if force_last_good:
        ...
        response = await fetch_tools.fetch_jsonl_async(download_uri)
    if not isinstance(response, list):
        try:
            backup = configuration.last_good_bulk_data.value
            return await fetch_tools.fetch_jsonl_async(backup), backup
...
def card_price(cardname: str) -> PriceDataType:
    return fetch_tools.fetch_json('http://prices.pennydreadfulmagic.com/{}/'.format(cardname.replace('//', '-split-')))
