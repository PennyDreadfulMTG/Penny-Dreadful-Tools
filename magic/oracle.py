...
def get_printings(generalized_card: Card) -> list[Printing]:
    sql = 'SELECT ' + (', '.join('p.' + property for property in card.printing_properties())) + ', s.code AS set_code, s.name AS set_name ' + ' FROM printing AS p' + ' LEFT OUTER JOIN `set` AS s ON p.set_id = s.id' + ' WHERE card_id = %s '
...
def get_printing(generalized_card: Card, setcode: str) -> Printing | None:
    if setcode is None:
        return None
    sql = 'SELECT ' + (', '.join('p.' + property for property in card.printing_properties())) + ', s.code AS set_code' + ' FROM printing AS p' + ' LEFT OUTER JOIN `set` AS s ON p.set_id = s.id' + f' WHERE card_id = %s AND (s.code = %s OR s.name LIKE {sqllikeescape(setcode)})' + ' ORDER BY s.code = %s DESC, s.released_at' + ' LIMIT 1'
...
def if_todays_prices(out: bool = True) -> list[Card]:
    ...
    """.format(not_clause=not_clause, format=current_format, prices_database=configuration.get('prices_database'), compare=compare, layouts=', '.join([sqlescape(lo) for lo in layout.playable_layouts()]))
