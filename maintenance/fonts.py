...
# $ PYTHONPATH=. uv run --frozen python3 maintenance/fonts.py show >/tmp/index.html && open /tmp/index.html
#
# Called with the options flag we will output an HTML file showing the symbols
# $ PYTHONPATH=. uv run --frozen python3 maintenance/fonts.py options >/tmp/index.html && open /tmp/index.html
...
def ad_hoc(*args: str) -> None:
    ...
    # Make a copy of all_graphemes for processing BUT exclude a bunch of common characters because we never want those
    ...
    # deck name – https://pennydreadfulmagic.com/decks/10989/ - see also https://pennydreadfulmagic.com/decks/264833/ and
    # https://pennydreadfulmagic.com/decks/265643/ which break '5' and '2'.
    # Would be nice to exclude everything in main-text but doing that makes the last two decks above only nonprinting chars for some reason.
    remaining_graphemes = {grapheme for grapheme in all_graphemes if not any(c in grapheme for c in "0123456789'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ")}
