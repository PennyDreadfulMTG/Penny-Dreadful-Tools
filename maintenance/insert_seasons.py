...
def run() -> None:
    existing = list(map(lambda s: s['id'], db().select('SELECT id FROM `season`;')))
    for season, setcode in enumerate(seasons.SEASONS, start=1):
        if season not in existing:
            ...
            if info.enter_date_dt < dtutil.now():
                ...
                db().execute('INSERT INTO season (id, code, start_date) VALUES (%s, %s, %s);', [season, setcode, dtutil.dt2ts(season_start)])
