...
def info(card):
    ...
    r['low'] = min(p[0] for p in rs)
    r['high'] = max(p[0] for p in rs)
    ...
    r['price'] = conn.cursor().execute(sql, [card.name]).fetchone()[0]
    ...
    r['week'] = conn.cursor().execute(sql, [week, card.name]).fetchone()[0]
    r['month'] = conn.cursor().execute(sql, [month, card.name]).fetchone()[0]
    r['rotation'] = conn.cursor().execute(sql, [last_rotation, card.name]).fetchone()[0]
